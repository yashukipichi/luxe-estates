...
function showLeadModal(onSuccess) {
  ...
  async function submitLead() {
    ...
    if (lib && lib.parsePhoneNumber) {
      try {
        const parsed = lib.parsePhoneNumber(phoneRaw, country);
        if (!parsed.isValid()) {
          const example = lib.getExampleNumber?.(country)?.formatNational() || '98765 43210';
          errEl.textContent = 'Please enter a valid phone number for the selected country (e.g. ' + example + ').';
          errEl.style.display = 'block';
